...
There are four types of data storage in Docker, namely as volumes, bind mounts, tmpfs mounts and containers writable layer.
For persisting data purposes, volumes and bind mounts are recommended as the other two only store data temporarily.

### __Store data within containers__
### Container and image layers
![image layers](https://docs.docker.com/storage/storagedriver/images/container-layers.jpg) 


Under the hood, each image is a stack of read-only binary layers, except the last top layer. This layer is often called container layer, since it is create and use by a container when it starts. All data stored in this layer will be destroyed when the container is removed. All image layers contents are managed with storage drivers. [Learn more](https://docs.docker.com/storage/storagedriver/)

![container layers](https://docs.docker.com/storage/storagedriver/images/sharing-layers.jpg)

The diagram above shows relationship between multiple containers with a same base image. Since each writable layer is created with the container itself, each container is now a different instance of the same base image with their own data state.
